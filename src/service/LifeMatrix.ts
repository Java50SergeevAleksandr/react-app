...
export default class LifeMatrix {
    ...
    nextStep(): number[][] {
        const getNumberOfLifeCells = (borders: Borders): number => {
        ...
        function getNumber(initNumber: number, col: number, row: number, array: number[][]): number {
            ...
            const numberOfLifeNeighbors = getNumberOfLifeCells(borders);

            return initNumber ? getNumberForLifeCell(numberOfLifeNeighbors-1) : getNumberForDeadCell(numberOfLifeNeighbors);
        }
